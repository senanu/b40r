...
5.  Connect the points using `type='b'`. Again, this is an argument to the `plot()` command and tells R to use **b**oth lines and points. You could use `type='l'` (the letter 'ell') for **l**ines and `type='p'` for **p**oints.
...
3.  Use the last line of this code chunk to get the fitted values for the Michaelis-Menten equation.
4.  Make sure the code works, then change `eval=FALSE` to `eval=TRUE`.
