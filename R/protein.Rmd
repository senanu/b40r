...
I strongly recommend that you read this file using the "Visual" rather than "Source" mode. However, if you need to send me code (if you get stuck), you'll need to switch back to "Source" before copy-and-pasting.

Before starting this lab, do a 'git pull' by clicking the blue down arrow in the git tab of the top-right pane. **This is important! Do it now!** This will download the latest version of the code, including this file. See [the Git section of the FAQ here](https://senanu.github.io/b40/docs/labs/R/r_faq.html#before-each-lab).

# Importing data

Get your data for the standard curve into R using a Google Sheets spreadsheet or any other method that you prefer. You'll need to **remember how to save your data from Google as a .csv file**. You can use the `read.csv()` function to get it into R. You can find a [reminder here.](https://senanu.github.io/b40/docs/labs/R/r_faq.html#why-cant-r-read-my-google-data-file)
...
4.  Change the `eval=FALSE` to `eval=TRUE` on the top line of the following code chunk once your code runs. This is necessary to knit properly.
...
-   Troubleshooting hints
    ...
    2.  The above command is written on multiple lines. R will interpret it as one command because the command doesn't end until the parentheses are closed. It is useful to break up functions with many arguments like this so that they are easy to read.
    3.  Make sure there is a comma between each argument. In the code above, this means that there should be a comma at the end of each line, since each argument takes up a line.
    4.  RStudio will automatically indent for you. This also makes it easier to read and figure out where a command (verb) starts and ends.
    5.  The values for main, xlab, and ylab should be in quotes, since you are just asking R to print some text on the graph.
...
## Don't do anything with any part of this code chunk.
# Make up some data
set.seed(1)
professors.data <- data.frame(x=1:10, y=rnorm(10) + (1:10)/2)
# Do a regression analysis and save it in an object called 'profs.regression'
profs.regression <- lm(professors.data$y ~ professors.data$x)
...
-   ***m*** The slope. This is given as the second of the coefficients. In this case, it is 0.5547.
-   ***b*** The y-intercept. This is conveniently labeled as the (Intercept). In this case it is -0.1688
...
3.  You can do several operations in one line. For example, taking the median of a set of values (from the `mtcars` dataset that we used previously) and then multiplying it by 10 and saving it into "median.value" might look like: `median.value <- median(mtcars$mpg) * 10`
4.  If the code runs without errors, change the `eval=FALSE` to `eval=TRUE`.
...
-   When you look at your graph, you should have blue and red dots, and they should be [on the regression line]{.underline}. There are a few reasons why you may see fewer than 10 dots (5 red and 5 blue):
    ...
    -   Some of the dots don't fit within the boundaries of the graph. Check your data in the "Calculate Protein Concentration" section above to see if this applies to your graph. If this is the case, go back to the beginning of this `Rmd` file to the section (Plot a standard curve) where you initially plotted the graph. Change limits on the x-axis by changing the `xlim = c(0,2)` so that the second value (`2` here) is large enough to accommodate your largest x-value. Similarly, change `ylim = c(0,1)` so that the second value (`1` here) is large enough for your largest y-value. After you make these changes, run [all]{.underline} the subsequent code so that your changes take effect on what you did later. Check again that all 10 dots are accounted for.
...
3.  Replace my name with your name at the top of this document.
4.  Recompile this document by clicking `Knit` then `Knit to html`. Save a copy of the resulting HTML file on your computer. You will turn in a copy of this file like you have done in the Intro_to_r and Intro_to_RStudio labs.
