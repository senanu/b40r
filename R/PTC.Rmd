...
knitr::include_graphics("../pics/PTC/PTClabFlowChart.png")
```
...
# Download DNA sequences from NCBI website {#download-dna-sequences-from-ncbi-website}
...
5.  At the top of the result, you can see that the file you are being shown is a 'GenBank' file, there is a reference number (it is called an "accession number" and in this case begins with "NM\_"). Also note that the gene is 1143 base pairs long (it's very short compared to most eukaryotic genes).'
...
knitr::include_graphics("../pics/PTC/genbank.png")
```
...
knitr::include_graphics("../pics/PTC/codonTable.png")
```
...
knitr::include_graphics("../pics/PTC/ORFs.png")
```
...
2.  Enter the accession number (NM\_...) that you wrote down when you first found the TAS2R38 gene into the search box. If you had DNA sequence, you could also enter that.
...
knitr::include_graphics("../pics/PTC/ORFs-TAS.png")
```

4.  Take a screenshot of the results. You will turn this in.
5.  In the output (Fig \@ref(fig:orfs-tas-pic)), you should see a pictoral representation of the sequence that you submitted. Note that:
    a.  it begins at base 1 and ends at 1143.
    b.  in all, there are 10 ORFs.
    c.  you can zoom in until you can see each nucleotide and each codon.
    d.  you can get the nucleotide sequence or the protein sequence (in single-letter codes). Select the longest ORF (it is probably labeled ORF1) and choose to view the nucleotide sequence.
        1.  What is the start codon?
        2.  What stop codon is used?
    e.  some ORFs (eg. ORF1) are on the 'plus' strand and some go in the other direction on the 'minus' strand. You can see this both in the table and as arrows on the map.
    f.  select to see the six-frame translation track by clicking the link at the bottom-right of the main map. You will see a picture similar to Fig \@ref(fig:orfs-pic). Zoom in to the picture using the 'magnifying glass' icon on the map.
6.  Close the six-frame translation track window.
7.  In many cases, the longest ORF will be the one that codes for a protein. On the map of the ORFs, right click the long ORF and select to "Set Sequence Origin At Feature", then accept the new origin of 85. This will just renumber the scale so that both are given.
...
2.  In the dialog box that pops up, select "Variation" on the left and then select "GMAF \>= 0.01" on the main box then click "Configure". See Fig \@ref(fig:tracks-dialog).
...
knitr::include_graphics("../pics/PTC/tracks-dialog.png")
```
...
3.  You will also see that the codon GCT codes for Alanine (shown as an "A" right below the "GCT" on the ORF1 track; confirm this in the codon table (Fig \@ref(fig:codons-pic)), though you should realize that the DNA strand shown is the "coding" strand of DNA. The complementary strand to the "coding" strand is the "template" strand, and it is the template strand that produces the mRNA that is used in the codon table. Since the mRNA and the "coding" strand are both complementary to the same "template" strand, their sequences are the same except for the fact that RNA uses uracil instead of thymine.)
...
knitr::include_graphics("../pics/PTC/PCR.png")
```
...
2.  **Annealing** Next, the temperature is cooled and primers complementary to particular sequences are allowed to anneal. These primer sequences are pointed toward each other on the complementary strands (see Fig \@ref(fig:PCR)).
...
knitr::include_graphics("../pics/PTC/PrimerBLASTParameters.png")
```

1.  Enter the accession number ("NM\_...") that you looked up at the beginning of this exercise into the main search box of [Primer-BLAST](https://www.ncbi.nlm.nih.gov/tools/primer-blast/). (See Fig \@ref(fig:primerBLASTParameters))
...
knitr::include_graphics("../pics/PTC/PrimerBLASTOutput.png")
```
...
knitr::include_graphics("../pics/PTC/Fnu4H1.png")
```
...
2.  Enter the sequence "GCNGC" in the search box at the top of the graphical view region (Fig \@ref(fig:primerBLASTOutput))
...
5.  Save a screenshot. You'll turn this one in as well. It will be similar to \@ref(fig:RestrictionSite).
...
knitr::include_graphics("../pics/PTC/RestrictionSite.png")
```
...
2.  Finally, check all the sequences you wrote against the sequence you downloaded in [step 4](#download-dna-sequences-from-ncbi-website). Notice that the sequence that you download is for the top, 5'-\>3' strand in Fig. \@ref(fig:PTCRestrictionMap).
...
knitr::include_graphics("../pics/PTC/PTCRestrictionMap.png")
```
...
knitr::include_graphics("../pics/PTC/loadingGel.png")
```
...
knitr::include_graphics("../pics/PTC/gelResults.png")
```
...
knitr::include_graphics("../pics/PTC/PTCGelPic.png")
```
...
2.  The results from searching for your restriction site in the PCR results. This will be similar to Fig. \@ref(fig:RestrictionSite).
3.  A map of restriction sites of your PCR product, showing the sizes of both alleles. This will also include the primers used. This will be similar to (or derived from) Fig. \@ref(fig:PTCRestrictionMap).
4.  A sketch of a gel similar to Fig. \@ref(fig:PTCGelPic) that includes the bands you'd expect for a heterozygote.
