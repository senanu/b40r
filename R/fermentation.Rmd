...
Please make sure you do a 'git pull' before you work on this assignment. In the top right pane, click the "git" tab, then click the blue down arrow. This will give you the most up-to-date code.

For this lab, I strongly recommend that you use the "Visual" mode rather than the "Source" mode to view this file.
...
## Use the bottom row for your own real data.
## You can switch between them by adding or removing the 
## 'hashtags' that R uses to denote a comment. R will ignore
## a line that has a hashtag at the beginning.
#dat <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vQ6FA2mzfFo3c-_yiyKuoqNf2UxssLel2pBM3tFoEbJWpola3DWbsq3uzsk3hlX7zIioG23JAJ31Smp/pub?gid=1495812031&single=true&output=csv") # use this line for some fake data
dat <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vQ6FA2mzfFo3c-_yiyKuoqNf2UxssLel2pBM3tFoEbJWpola3DWbsq3uzsk3hlX7zIioG23JAJ31Smp/pub?gid=32807751&single=true&output=csv") # Use this line to grab the real class data once it has been input.
```
...
ggplot2::ggplot(data = salt,
  ...
  xlab("Time (minutes)") +
  ylab(expression(Volume~of~CO[2])) +
  ggtitle("This graph uses fake data") +
  labs(color = "Carbohydrate")
```

## Is this line flat?

The line above (with fake data) isn't quite level -- the slope doesn't seem to be quite 0. In statistics, we would state a "null hypothesis" (H~0~) that states that there is no relationship between time and the volume of CO~2~ produced. An alternate hypothesis (H~A~) would state that there is a relationship between the two:
...
Remember that this uses the formula for a straight line $$Y = mX + b$$ where $Y$ is CO~2~ and $X$ is Time. $m$ is therefore the slope and $b$ is the intercept. Let's use the `summary()` function to get more information about this linear regression:
...
Please make sure you are using your own data, and not the fake data I provided for this lab. In the "Get Data" section above, make sure the second of the two 'read.csv' commands is UNcommented (no hashtag) and the first is commented out (has a hashtag).
...
```         
Type your answer here. Make sure that it is within the 3 'backticks' so that it appears in a box in the HTML document after knitting. (Note that the backticks are only visible when you view the document in "source" mode. In "visual" mode, it should appear within a box.)
```
...
In the next bit of code, we'll compare glucose with starch. Look at the colorful graph above and make a prediction about whether you think there is a difference in slope between the glucose and starch lines.

```{r gluc-starch-reg, echo=FALSE}
gluc.starch <- dat[dat$carb == 'glucose' | dat$carb == 'starch',] # "|" means "or"
gluc.starch$carb <- as.factor(gluc.starch$carb)
contrasts(gluc.starch$carb) <- c(-0.5, 0.5) # see why this is necessary:  https://towardsdatascience.com/how-to-correctly-interpret-your-continuous-and-categorical-variable-interactions-in-regressions-51e5eed5de1e
gluc.starch.regression <- lm(co2vol ~ time * carb, data = gluc.starch)
summary(gluc.starch.regression)
```

What do your data show regarding the difference in fermentation between sucrose and starch? Is the p-value in the last row of the Coefficients table less than 0.05 $(p<0.05)$?
...
What does this tell you about the different enzymes that yeast has to break down these different molecules?
