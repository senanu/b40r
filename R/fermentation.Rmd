...
Remember that this uses the formula for a straight line $$Y = mX + b$$ where $Y$ is CO~2~ and $X$ is Time. $m$ is therefore the slope and $b$ is the intercept. So we could write this as $$ Y=`r round(salt.regression$coefficients[2],4)`X + `r round(salt.regression$coefficients[1],4)`$$
