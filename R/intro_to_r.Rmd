...
As you have already learned in a previous exercise, this is an R Markdown document, which mixes text with code. To start out, you should 'knit' the document before making any changes. Please let me know if the file cannot be 'knitted' **before** any changes are made.
...
2.  View it in the visual editor to make the formatting nice and avoid seeing all the formatting code. **This is how I recommend you read the document**. Click the "visual" button to the top left of the top left (code editor) pane.

3.  Knit the document and read the HTML. This is the final form of the document but some things that you'll need get hidden in this form, so I don't recommend it. You can view it either in a separate window or in the "Plots and files" pane (lower right).
...
# Put your code "10 + 5" below this line. Don't include quotations. Then, run the line with 'cntrl-enter' or by clicking the green arrow at the top right of this code chunk.
...
a <- 2 # assign the value 2 into the variable 'a'
```
...
You specify which column you want using the dollar sign (`$`). ![First rows of mtcars dataset](../pics/02_cars.png)

We can use this dollar sign notation to isolate one column of data. For example, to calculate the mean (average) fuel economy for these cars, you can do:

```{r mpg, echo=FALSE, eval=TRUE}
print("Show only the 'mpg' (miles per gallon) column:")
mtcars$mpg 
print("get the mean (average) mpg for this dataset:")
mean(mtcars$mpg)
print("Save the mpg data into a separate vector called 'mileage'")
mileage <- mtcars$mpg
```
...
# What code would you use to get the mean (average) horsepower?
...
Since R is a computer program, it is very finicky about spelling. If you spell something wrong, it won't know what you are talking about! For example, typing `mtcars$mbg` instead of `mtcars$mpg` will generate an error because there is no data column called `mbg` in the `mtcars` dataset.
...
![Google sheets file with 3 columns of data](../pics/02_google_data.png){height="4in"}
...
![Publishing to the Web](../pics/02_publish_to_web.png){height="4in"}

![URL is in blue ready to copy/paste](../pics/02_publish_to_web_URL.png){height="4in"} --\>
...
Notice in the `read.csv()` command below that the command has parentheses, and the information that goes within the parentheses is the URL given to you by Google. You need to put that entire URL into quotes (""). For example:
...
2.  Add wet ingredients to bowl
3.  Stir to make thick batter
4.  Place batter in 9x13in pan
5.  Place pan in oven for 35 minutes at 350F
...
2.  Make a data file (I recommend Google Sheets).
...
3.  Write code in the empty code chunk below to read a data file (such as Google sheets).
...
4.  Make R print out the data by simply typing "mydata" alone on a line.
...
2.  Note that you should have previously (in a step above) saved your data into a variable called "mydata".
3.  Put some text between the quotes to label the x-axis and the y-axis
4.  Put some text between the quotes to label the entire graph (main)
5.  Change the 'eval=FALSE' to 'eval=TRUE' so that this graph will be included in the knitted output.
...
2.  Remember to clean your environment and clear the knitr cache. If you forgot how to do this, [look here](https://senanu.github.io/b40/docs/labs/R/intro_to_rstudio.html#what-to-turn-in). You don't need to do this every time you knit, just the last time as you prepare your html file to turn in.
3.  Knit the document to produce an html file.
4.  Check that the html file has **your** graph near the bottom.
5.  Save the html file.
6.  Turn in your html file on canvas. I will be looking to make sure that you completed the exercises section. Specifically, you **should have a simple graph using your own data.**
