...
There are 3 main ways in which the volume of your respirometers will change. 1. Respiration will use up Oxygen and replace it with Carbon dioxide. However, the carbon dioxide will be removed by the KOH and so will not contribute to the volume. 2. The temperature of the tube may change. We will try to control for this by having a control tube with no seeds and only inert beads. We can correct for this with the control. 3. The volume may change if you move the tube around after you start the experiment. This is mostly because the stopper is flexible and if you flex it, the interior volume may change slightly. Unfortunately, this is a random error for which we can't correct.

We are interested in measuring #1 above, but we can correct for #2 by subtracting the volume of the empty tube from the volume of each the other tubes. 1. Subtract the volume of the empty tube from the volume of the 'test' tube (either dry or wet seeds). I have done the first pair for you, but you'll need to do this for all the tubes. We will call the results of these calculations as 'net' change. 2. When this code block runs, change `eval=FALSE` to `eval=TRUE`.
...
```{r plot, include=TRUE, echo=FALSE, eval=FALSE}
